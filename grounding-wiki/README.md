...
Reference deployment: the reference-repo repo's `docs/wiki/` (22 notes + a repo-local bash port of
the gate). Downstream consumers: analyze-vault (Q&A), codebase-to-course (brief grounding),
educate (lesson grounding), reference-repo ingestion (MCP-served Q&A).
