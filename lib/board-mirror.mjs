...
const LINK_KEYS = ["id", "status", "specDir", "acs", "labels", "dispatches", "observedAt", "observedSha"];
...
export function validateMirror(mirror) {
  ...
  mirror.links.forEach((link, i) => {
    ...
    // dispatches is OPTIONAL (spec 066 R3), same additive contract as labels above: a mirror
    // written before this field existed has no key at all and stays valid.
    if (link.dispatches !== undefined) {
      if (!Array.isArray(link.dispatches)) {
        problems.push(`${where}.dispatches: expected array, got ${typeof link.dispatches}`);
      } else {
        link.dispatches.forEach((d, k) => {
          if (typeof d !== "string") problems.push(`${where}.dispatches[${k}]: expected string, got ${typeof d}`);
        });
      }
    }
...
/**
 * The dispatch-record marker (spec 066 R3), a deliberate sibling of `MARKER` above:
 *
 *   Dispatch: tier=<tier> pinned=<model-id> served=<model-id>
 *
 * One line per dispatch, anywhere in the card's text (description or comments) — a task
 * dispatched phase-by-phase carries several. Global + multiline, so every line is collected.
 *
 * WHY THIS EXISTS AT ALL: an inline-implemented task leaves artifacts *identical* to a
 * dispatched one — same commits, same specs, same ticks — so `served=` is the only residue
 * that distinguishes them. `tiers.mjs --check` proves a definition says `sonnet`; it cannot
 * see whether a dispatch happened. Field case: 2026-09-10, Lane 4 of the TASK-108 sweep ran a
 * whole task inline on the orchestrator's Opus session while three written sources said to
 * dispatch (`docs/design/lane-4-dispatch-gap.md`).
 *
 * This module only TRANSPORTS the raw payloads; the rule about what makes one valid (a real
 * `served=` value, not a placeholder) lives in `spec-bridge/gates/bridge.mjs`, which is the
 * half that judges. Same split as everywhere else here.
 */
const DISPATCH_MARKER = /^Dispatch:[ \t]*(\S.*?)[ \t]*$/gm;

/** Every `Dispatch:` line's payload, in document order. [] when there are none. */
export function parseDispatchRecords(raw) {
  return [...String(raw ?? "").matchAll(DISPATCH_MARKER)].map((m) => m[1]);
}
...
export function parseLinkedTask(raw) {
  ...
  const dispatches = parseDispatchRecords(text);
  const linked = { id, status, specDir: marker[1], acs };
  if (labels.length) linked.labels = labels;
  if (dispatches.length) linked.dispatches = dispatches;
  return linked;
}
...
export function projectBacklog(root) {
  return findLinkedTasks(root).map(({ id, status, specDir, acs, labels, dispatches }) => {
    const link = { id, status, specDir, acs };
    if (labels) link.labels = labels;
    if (dispatches) link.dispatches = dispatches;
    return link;
  });
}

/**
 * Regenerate `<root>/.board/links.json` for its own provider: read → replace `links` +
 * `generatedAt` → `validateMirror` → write. Refuses to write an invalid mirror (throws; the
 * on-disk mirror, if any, is left untouched).
 *
 * THE ENVELOPE CONTRACT — why this exists instead of a one-liner (spec 071 R3): a mirror is
 * `{ schema, provider, generatedAt, links }`, not a bare links array. `provider.project(root)`
 * (e.g. `projectBacklog`) returns ONLY the links array, so `writeMirror(root,
 * projectBacklog(root))` writes a file with no `schema`/`provider`/`generatedAt` at all — which
 * `readMirror` then rejects as `unknown schema undefined`, a malformed mirror masquerading as an
 * unrelated bug. Always regenerate through this function (or replicate its whole
 * read-replace-validate-write shape); never hand `writeMirror` a bare projection.
 *
 * Provider resolution: the existing on-disk mirror's `provider` field when one exists (so a
 * stale mirror regenerates as the provider it already claims to be); otherwise `.board.json`'s
 * `provider` (`loadBoardConfig`, defaults to `"backlog"` when absent).
 *
 * `requiresSync: true` (Jira — no local recompute exists, see `providers` below) is handled
 * explicitly: this throws naming the provider and pointing at its MCP-backed sync path
 * (`spec-bridge:board-sync`) rather than crashing on `provider.project(root)` being `null`.
 */
export function regenerateMirror(root) {
  const existing = readMirror(root);
  const providerName = existing ? existing.provider : loadBoardConfig(root).provider;
  const provider = providers[providerName];
  if (!provider) throw new Error(`board-mirror: unknown provider "${providerName}"`);
  if (provider.requiresSync)
    throw new Error(`board-mirror: provider "${providerName}" requiresSync — no local recompute exists; refresh it via its sync path (e.g. spec-bridge:board-sync), not --write`);

  const base = existing ?? { schema: CURRENT_SCHEMA, provider: providerName };
  const mirror = { ...base, links: provider.project(root), generatedAt: new Date().toISOString() };
  const problems = validateMirror(mirror);
  if (problems.length)
    throw new Error(`board-mirror: refusing to write an invalid mirror:\n${problems.map((p) => `  - ${p}`).join("\n")}`);
  return writeMirror(root, mirror);
}
...
/* ── --check / --write CLI (spec 052 R5, spec 071 R3), dual-use via lib/cli.mjs's runAsCli
 * guard ──
 *
 * node lib/board-mirror.mjs --check --root <dir>   # read-only: report drift/staleness
 * node lib/board-mirror.mjs --write --root <dir>   # regenerate via regenerateMirror() above
 *
 * `--write` is the ONLY supported regenerate entry point (spec 071 R3) — it never runs as a
 * side effect of `--check`. Nothing in this file self-heals: a stale/drifted mirror is reported
 * and left exactly as found until `--write` is run deliberately.
 *
 ...
 * FINDING (1), not an env error — it is the artifact that is broken, not the invocation. For
 * `--write`, a `requiresSync` provider is an env error (2) — no local recompute exists for it,
 * see `regenerateMirror`; an invalid resulting mirror is also refused (2) since nothing was
 * written.
 *
 ...
 *
 * R2's name-its-fix obligation: every FAILED line below is followed by a `fix:` line pointing
 * at a real, copy-pasteable command — `--write` for a recomputable drift, the provider's own
 * sync path (spec-bridge:board-sync) for a requiresSync staleness `--write` cannot address —
 * and the wording is deliberately distinct from the bridge gate's "task status lags its spec"
 * finding: this is about the MIRROR FILE, not the board. */
if (runAsCli(import.meta.url)) {
  const args = process.argv.slice(2);
  const rootIdx = args.indexOf("--root");
  const write = args.includes("--write");
  const check = args.includes("--check");
  if ((!write && !check) || (write && check) || rootIdx === -1 || !args[rootIdx + 1]) {
    console.error("usage: node lib/board-mirror.mjs (--check|--write) --root <dir>");
    process.exit(2);
  }
  ...
  if (write) {
    try {
      const path = regenerateMirror(root);
      console.log(`board-mirror: wrote ${path}`);
      process.exit(0);
    } catch (e) {
      console.error(`board-mirror --write FAILED: ${e.message}`);
      process.exit(2);
    }
  }
  ...
  if (provider.requiresSync) {
    ...
    if (stale) {
      ...
      console.log(`  (this is the MIRROR FILE lagging the "${mirror.provider}" board, not a task's board status lagging its spec)`);
      console.log(`  fix: refresh it via its sync path, e.g. /spec-bridge:board-sync (--write cannot recompute a requiresSync provider)`);
      process.exit(1);
    }
  ...
  if (expected !== actual) {
    ...
    console.log(`  (this is the MIRROR FILE lagging the board, not a task's board status lagging its spec)`);
    console.log(`  fix: node lib/board-mirror.mjs --write --root ${rootArg}`);
    process.exit(1);
  }
