...
export function deriveSpecState(specDir, { requireAnalysis = false } = {}) {
  ...
  const phases = parseTasks(read("tasks.md"));
  ...
  if (has("spec.md")) {
    status = has("plan.md") && tasksTotal > 0 && tasksDone === tasksTotal && analysisOk
      ? STATUS.DONE_ELIGIBLE
      : STATUS.IN_PROGRESS;
  }

  return {
    status, phases, tasksDone, tasksTotal, progressNote: progressNote(phases),
