...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Plugin scaffolded per docs/skill-patterns.md checklist (plugin.json, marketplace entry, README row, lib symlink) and passes check-docs.mjs + gen-marketplace --check + sync-version --check
- [ ] #2 SKILL.md orchestrator covers all six phases in gate->work->gate shape with everything-optional grounding detection, structured evaluator report template, steering-round protocol, isolation-safe cross-grounding, synthesis section contract, and inline fallbacks for every referenced script
- [ ] #3 Run tracker (scripts/run.mjs begin/finish/abandon) writes .handoff/reorient/runs at the invoking root only; gates/ verifies runs read-only (manifest vs artifacts: per-branch analyses, synthesis sections, decisions recorded) and never writes
- [ ] #4 Tests under test/ cover the run tracker and gate; node --test green
