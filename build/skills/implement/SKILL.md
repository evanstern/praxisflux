...
reference). Keeping build separate means the thing that has the context to say
...
The response file is your **entire** return: do **not** touch the producer's tracked state.
In particular, never write educate's `progress.json` — recording the evidence
(`handoff.returned=true`, status `built`) is the producer's return-leg job, done when it picks
up your response (evidence is always the consumer's to record, in its own ledger).
...
- It does not write the producer's ledger (`progress.json` or any tracked state) — the
  producer records `handoff.returned` + status `built` when it consumes the response.
