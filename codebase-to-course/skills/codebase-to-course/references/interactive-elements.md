...
**Comments-on-top rendering (automatic):** at every screen size, `main.js` rebuilds each translation block as a single wide code panel (~100 mono characters on desktop; it breaks out of the content column) with every `.tl` note interleaved as a `//`-style comment line ABOVE its code line, plus a per-block "hide notes" toggle for a pure-code read. The builder also reformats the code for display — tokens stay byte-identical, but the snippet's common leading indentation is stripped, nesting is re-expressed as 2 columns per tab, indents become padding (so wrapped continuations align under the code, never at column 0), and each note is indented to match its code line. So: still paste code verbatim, deep source indentation and all — layout is normalized at render time. The side-by-side markup you author is the no-JS fallback and the data source — so keep authoring it exactly as specified above. Pairing is positional: note *i* sits above `.code-line` *i*, so **author exactly one `.tl` per `.code-line`, in the same order**. If a thought spans two code lines, keep the note on the first and give the second a short continuation note ("…and close the loop").
