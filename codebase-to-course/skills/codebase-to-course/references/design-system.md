...
## Dark Mode (built into the prebuilt assets)

`styles.css` ships a complete dark theme under `:root[data-theme="dark"]` — warm charcoal (#171412 base), never cold gray. `main.js` injects a 🌙/☀️ toggle into the nav, defaults to the OS preference, persists explicit choices in `localStorage` (`course-theme`), and `_base.html` sets `data-theme` before first paint so there's no flash. **None of this needs authoring or customization per course.**

What module authors must know:
- Dark mode works by **overriding tokens**, so module HTML must only ever color things with tokens (`var(--color-surface)`, `var(--color-text-secondary)`, `var(--color-accent-light)`, …). A hardcoded `#FFF` or pastel hex will glare in dark mode.
...
- `styles.css` also ships the **praxisflux shared token schema** as aliases (`--ink`, `--muted`,
  `--accent`, `--line`, `--chip`, `--callout-bg`, …) mapped onto the course tokens. Snippets
  written against the shared names — e.g. praxisflux `lib/toolkit` modules — drop in unchanged and
...
`main.js` self-builds a fixed left-rail ToC from the `.module` sections (titles read from each `.module-title`, falling back to nav-dot tooltips). It scroll-spies the active module and collapses via a "hide" button (a slim ☰ tab reopens it; state persists in `localStorage` as `course-toc-hidden`). Below 1280px it becomes a slide-out drawer under the header, toggled by a ☰ button that `main.js` injects into the nav (closes on link tap or tap-outside). No per-course authoring — it appears automatically when a course has 2+ modules with proper `.module-title` headings.
