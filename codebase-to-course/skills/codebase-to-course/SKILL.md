...
- **Dark mode + ToC come free**: the prebuilt `styles.css`/`main.js`/`_base.html` include a warm dark theme (nav toggle, OS default, persisted) and a self-building collapsible table of contents (desktop side rail; slide-out drawer from the nav's ☰ on smaller screens). Nothing to author per course — but module HTML must color everything via tokens (never hardcoded light-only hexes), or dark mode breaks. See `references/design-system.md` > "Dark Mode".
