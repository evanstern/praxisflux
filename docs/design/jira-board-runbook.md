...
| 2026-08-28 | TASK-109 | _in flight_ | — | — | **In flight (Lane 1).** Claim `415d5c8` (atomic: status flip + 4 phase ACs seeded from spec 052 tasks.md; the spec dir and Spec marker already existed on main, hand-authored under the escape line). Worktree `.claude/worktrees/task-109`, branch `task-109-board-mirror` off `679492c`, pushed. Tier sonnet · `cc/claude-sonnet-5[1m]`. **phases: 1 done (`14b4577`, +9 tests), 2 done (`5580d51`, parser moved, bridge.mjs −46 lines, AC#9 protected tests byte-identical), 3 done (`412c935`, staleness + provider registry + projector, +7 tests, no provider-name conditional and fail-closed on unknown provider both verified), suite 465/465 — all verified by the orchestrator; 4 dispatched (CLI, dogfood, version bump, ~18 re-pins).** Served-model: no ledger spot-check required — this runbook's rule asks for one only if the tier config changed since TASK-107, and `tiers.mjs --check` exited 0 with all three tiers `unchanged`. (Noted for future sessions: the router admin API at `127.0.0.1:20128/api/requests` rejects `ANTHROPIC_AUTH_TOKEN` with 401 — it uses a separate auth scheme, so ledger evidence needs the router's own token from the operator.) |
