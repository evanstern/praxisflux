...
Generated by grounding-wiki `scripts/capsules.mjs` at corpus commit e87f7caa1b9edadf94fdaab5db8d4c0ebb10ae8f.
...
- [[test-suite-catalog-plugins-gates-pdlc]] — per-file coverage, pdlc's own gate suites: the plant/tiers surface and the opt-in root-guard hook
  Per-file coverage of pdlc's own gate suites — the plant/tiers surface (test/pdlc.test.mjs) and the two opt-in PreToolUse hooks (test/root-guard-hook.test.mjs, test/root-guard-scan.test.mjs, test/read-size-gate.test.mjs). Split summary-style from test-suite-catalog-plugins-gates, whose remaining files it still catalogs directly.
